...
class Conversacion:
    ...
    def _generar_id_sesion(self) -> str:
        # Un id de sesión por ejecución del programa, basado en hora exacta.
    ...
    def contexto_reciente(self, n: int = 6) -> str:
        """Devuelve el texto concatenado de los últimos n mensajes, para dar contexto."""
        recientes = self.mensajes[-n:]
        return " ".join(m["texto"] for m in recientes)

    def exportar(self, ruta_destino: str):
        """Exporta la conversación actual a una ruta arbitraria (fuera de la carpeta de datos)."""
        import json
    ...
    def listar_conversaciones(almacenamiento: AlmacenamientoJSON) -> list:
        """Lista todas las conversaciones guardadas (fecha, id_sesion, nº mensajes)."""
